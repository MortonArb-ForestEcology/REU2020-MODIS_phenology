...
greenup.out <- list()
for(i in 1:nrow(npn.pts)){
  greenup.out[[i]] <- MODISTools::mt_subset(product="MCD12Q2", band=c("Greenup.Num_Modes_01", "MidGreenup.Num_Modes_01"), start= '1999-01-01', end= '2019-12-31', lat = npn.pts$latitude[i], lon=npn.pts$longitude[i], site_name = npn.pts$site_id[i])
}
...
ggplot(data=leaf.MODIS) +
  facet_grid(BAND~.) +
  geom_histogram(aes(x=greenup.yday))
...
#   greenup.out[[i]] <- MODISTools::mt_subset(product="MCD12Q2", band="MidGreenup.Num_Modes_01", start= '1999-01-01', end= '2019-12-31', lat = npn.pts$latitude[i], lon=npn.pts.$lon[i], site_name = budburst.pts$site_name[i])
# }
# head(leaf.MODIS)
# leaf.MODIS <- dplyr::bind_rows(greenup.out)
# summary(leaf.MODIS)
...
write.csv(leaf.MODIS, file.path(path.MODIS, paste0("MODIS_", species.name, ".csv")), row.names=F)
