...
#dat.tst$value <- (dat.tst$value/days.mrk$mrk.yday)
...
ggplot(data= dat.tst, mapping= aes(x= greenup.year, y= greenup.yday)) + 
  ggtitle('                                           Ivy Branch Farm Greenup Date From 2005 - 2015') +
  geom_line(color= 'orange') +
  geom_point(mapping= aes(size= 1, label= band.name)) + 
  geom_text(label= dat.tst$greenup.yday, color= 'black', size = 5) +
  scale_x_continuous('YEAR', breaks= seq(from= '2005', to= '2015', by= 1 )) +
  scale_y_continuous('15% GREENUP POINT (YDay)', breaks= days.mrk$mrk.yday, labels= days.mrk$Label)
